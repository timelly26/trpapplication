...
export const FeesBreakdown = ({ totalFee, amountPaid, remainingFee, payments = [] }: Props) => {
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-4 sm:p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                {payments.length > 0 && (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition-colors"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                    <p className="text-xs text-red-300/70 uppercase tracking-widest font-bold">Remaining</p>
            ...
            {feeBreakdown.size > 0 ? (
                    ...
                    <table className="w-full text-left min-w-[320px]">
                        ...
                        <tbody className="text-sm">
                            {Array.from(feeBreakdown.entries()).map(([feeType, data]) => {
                                ...
                                return (
                                    <tr key={feeType} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="text-center py-8 text-gray-500 text-sm">
                    No fee breakdown data available yet.
                </div>
            )}
            ...
            <div ref={receiptRef} className="hidden">
                <div className="p-8 bg-white text-black" style={{ width: "210mm", minHeight: "297mm" }}>
                    <div className="mb-8 text-center">
                        ...
                        <p className="text-gray-600 mt-2">Official Payment Receipt</p>
                    </div>
                    ...
                    <div className="mb-8 border-b-2 border-gray-300 pb-4">
                        ...
                        <div className="bg-gray-100 p-3 rounded">
                            <p className="text-sm text-gray-700">Payment Progress: <span className="font-bold">{Math.round(paidPercentage)}% Complete</span></p>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    {payments.map((payment) => (
                                        <tr key={payment.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 p-2 text-sm">
                                                <span className={`px-2 py-1 rounded text-white text-xs font-semibold ${payment.status === "completed" ? "bg-green-600" : "bg-amber-600"
                                                    }`}>
                                                    {payment.status}
                                                </span>
                                            </td>
